...
init python:
    ...
    anime_dir = os.path.join(config.gamedir, "images", "AnimeStyle")
    anime_bg_list = [ ]

    if os.path.isdir(anime_dir):
        for fn in sorted(os.listdir(anime_dir)):
            if fn.lower().endswith((".png", ".jpg", ".jpeg")):
                anime_bg_list.append(os.path.join("images/AnimeStyle", fn))
...
style say_dialogue is drop_shadow_1
...
screen main_menu():
    ...
    # AnimeStyle 自动轮播背景（随机擦除效果，每 1.92 秒切换）
    if len(anime_bg_list) > 0:
        add "carousel_anim"
        ...
        # 没有 AnimeStyle 图片时，回退到默认主菜单背景
        add Transform("gui/main_menu.png", 
    ...
    vbox:
        ...
        xoffset -120
        yoffset 100
        text "{size=30}Ver.[config.version]{/size}" font "fonts/hk4e_zh-cn.ttf":
...
screen game_menu(title, scroll=None, yinitial=0.0, spacing=0):
    ...
    # 游戏内菜单使用截图背景，主菜单中使用主菜单背景
    if not main_menu:
        # 在游戏内，使用自定义背景图片
        add "gui/ingame_menu.png":
            size (config.screen_width, config.screen_height)
            xalign 0.5
            yalign 0.5
    else:
        # 在主菜单中，使用主菜单背景
        add Transform(gui.main_menu_background,
            size=(config.screen_width, config.screen_height),
            xalign=0.5,
            yalign=0.5
        )
...
screen about():
    ...
    use game_menu(_("关于"), scroll="viewport"):
        ...
        vbox:
            ...
            label "{size=40}[config.name!t]{/size}"
            ...
            text _("{size=20}{color=#777777}\n\n\n版本号：[config.version!t]\n引擎：{a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]\n[renpy.license!t]{/color}{/size}")
...
screen nvl(dialogue, items=None):
    window:
        ...
        at transform:
            on show:
                alpha 0.0
                linear 0.1 alpha 1.0  # 0.5秒淡入
            on hide:
                linear 0.1 alpha 0.0  # 0.5秒淡出


        # 使用 vbox 包裹所有内容，并设置垂直居中
        vbox:
            yalign 0.5        # 垂直居中
            xfill True        # 横向占满，方便内部文字对齐
            
            # 【关键】直接在这里控制每句对话之间的间距
            # 如果 gui.nvl_spacing 没效果，直接改成数字，例如 spacing 50
            spacing (gui.nvl_spacing if gui.nvl_spacing else 40)
